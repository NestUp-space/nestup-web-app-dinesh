...
const isConfigCorrect = validator();

if(!isConfigCorrect){
  Logger.info("Closing server as not able to connect to cache.")

  process.exit(0);
}

const redisURL = `${redis.protocol}://${redis.username}:${redis.password}@${redis.host}:${redis.port}`;
