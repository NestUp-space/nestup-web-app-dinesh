 ...
 *
 * Rules:
 * | Thickness  | Exterior    | Inner       |
 * |------------|-------------|-------------|
 * | 16 - 17 mm | 22mm x 2.0mm | 22mm x 0.8mm |
 * | 18 - 21 mm | 25mm x 2.0mm | 25mm x 0.8mm |
 * | 22 - 27 mm | 30mm x 2.0mm | 30mm x 0.8mm |
 * | > 27 mm    | 45mm x 2.0mm | 45mm x 0.8mm |
 */
function getEbSize(thickness, isInner) {
  ...
  var ebThickness = isInner ? "0.8mm" : "2.0mm";
...
function processMaterialSummary(sourceSheet) {
  ...
  for (var i = 1; i < summaryValues.length; i++) {
    ...
    // ------------------------------------------
    // CASE 1: Mixed outer + specific inner (includes Case H: "122 & 123 Inner")
    // Examples: "122 @ 123SF Inner", "color & 124 SF Inner", "1234 @ 5678 & 143 Inner"
    // ------------------------------------------
    if (hasInner && (materialString.includes('@') || materialString.includes('&'))) {
      var parts = materialString.split(/[@&]/).map(function(p) { return p.trim(); });
      var innerColorCode = extractInnerColorCode(materialString, true);

      // ERROR: Invalid pattern e.g. "122 @ Inner" - inner part must have a color code
      if (!innerColorCode || innerColorCode === 'NA' || innerColorCode.trim() === '') {
        throw new Error('Invalid material naming: cannot use @ with Inner. Use a specific inner color (e.g. "122 @ 123 Inner") or use "122" for generic inner.');
      }

      var specificInnerColor = innerColorCode + ' Inner';
      var exteriorParts = parts.slice(0, parts.length - 1);

      // Add each EXTERIOR colour laminate (handles 2-part and 3+ part e.g. "1234 @ 5678 & 143 Inner")
      var ebPerExterior = exteriorParts.length > 0 ? totalEb / exteriorParts.length : 0;
      for (var p = 0; p < exteriorParts.length; p++) {
        var extColor = getCleanColorCode(exteriorParts[p]);
        if (extColor && extColor !== 'NA') {
          if (!laminateSummary[extColor]) {
            laminateSummary[extColor] = {
              exteriorSheets: 0,
              innerSheets: 0,
              colorCode: extColor,
              rooms: new Set()
          ...
          roomsToAdd.forEach(function(r) { laminateSummary[extColor].rooms.add(r); });
    ...
    // ------------------------------------------
    // CASE 3 (Case B): Front design with @ - multiple exterior colours + generic inner
    // Example: "1234 @ 567" → 1234: ×1, 567: ×1, Inner: ×1
    // ------------------------------------------
    if (materialString.includes("@") && !hasInner) {
      var atParts = colorCode.split(/\s*@\s*/).map(function(x) { return x.trim(); }).filter(function(x) { return x && x !== 'NA'; });
      var ebPerColor = atParts.length > 0 ? totalEb / atParts.length : 0;

      for (var a = 0; a < atParts.length; a++) {
        var atColor = atParts[a];
        if (!laminateSummary[atColor]) {
          laminateSummary[atColor] = {
            exteriorSheets: 0,
            innerSheets: 0,
            colorCode: atColor,
            rooms: new Set()
        ...
        roomsToAdd.forEach(function(r) { laminateSummary[atColor].rooms.add(r); });
