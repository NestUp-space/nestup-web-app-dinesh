...
function _getPlanksFromSheet() {
  ...
  for (let i = 1; i < formattedData.length; i++) {
    ...
    if (plankId) {
      const operations = {};
      const existingDetails = plankDetailsMap.get(plankId) || { operations: {} };
      ...
      // --- MODIFIED: Store EB Value in Map ---
      const ebVal = ebIndex > -1 ? (parseFloat(row[ebIndex]) || 0) : 0;
      plankDetailsMap.set(plankId, { operations, ebValue: ebVal });
    }
  }
  ...
  for (let i = 1; i < plankListData.length; i++) {
    ...
    const details = plankDetailsMap.get(uniqueIdForLookup) || { operations: {}, ebValue: 0 }; 
